...
export async function GET(
  ...
  const [row] = await prisma.$queryRawUnsafe<BoletaRow[]>(
    `SELECT b.folio, b.fecha, e.rut AS empresa_rut
     FROM "${ctx.schemaName}".boletas b
     JOIN public.empresas e ON e.id = $1
     WHERE b.id = $2 AND b.folio IS NOT NULL`,
    ctx.empresaId, params.id
  )

  if (!row) return NextResponse.json({ error: "Documento no encontrado" }, { status: 404 })

  const key = buildR2Key(row.empresa_rut, 39, row.folio!, new Date(row.fecha), "pdf")
