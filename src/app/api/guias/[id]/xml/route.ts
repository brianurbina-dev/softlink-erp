...
export async function GET(
  ...
  const [guia] = await prisma.$queryRawUnsafe<{ folio: number | null; fecha: string }[]>(
    `SELECT folio, fecha FROM "${ctx.schemaName}".guias WHERE id = $1`,
    params.id
  )

  if (!guia) return new NextResponse("Guía no encontrada", { status: 404 })
  if (!guia.folio) return new NextResponse("Guía sin folio — no tiene XML almacenado", { status: 404 })
  ...
  const xmlKey = buildR2Key(empresa.rut, 52, guia.folio, new Date(guia.fecha), "xml")
  ...
  return new NextResponse(content, {
    headers: {
      ...
      "Content-Disposition": download
        ? `attachment; filename="DTE_52_${guia.folio}.xml"`
        : "inline",
    },
  })
}
