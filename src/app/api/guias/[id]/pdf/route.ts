...
export async function GET(
  ...
  const [row] = await prisma.$queryRawUnsafe<GuiaRow[]>(
    `SELECT g.folio, g.fecha, e.rut AS empresa_rut
     FROM "${ctx.schemaName}".guias g
     JOIN public.empresas e ON e.id = $1
     WHERE g.id = $2 AND g.folio IS NOT NULL`,
    ctx.empresaId, params.id
  )

  if (!row) return NextResponse.json({ error: "Documento no encontrado" }, { status: 404 })

  const key = buildR2Key(row.empresa_rut, 52, row.folio!, new Date(row.fecha), "pdf")
