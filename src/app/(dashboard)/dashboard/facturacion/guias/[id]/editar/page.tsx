...
function genKey(): string { return Math.random().toString(36).slice(2, 10) }
function emptyItem(): ItemForm {
  return { key: genKey(), productoId: "", descripcion: "", cantidad: 1, precioUnitario: 0, descuento: 0 }
}
...
function fmtMonto(n: number): string { return `$ ${n.toLocaleString("es-CL")}` }

const TIPOS_TRASLADO = [
  { value: 1 as const, label: "Venta (tipo 1)", desc: "Traslado por venta — aplica IVA 19%" },
  { value: 2 as const, label: "Traslado interno (tipo 2)", desc: "Traslado sin venta — sin IVA" },
  { value: 3 as const, label: "Consignación (tipo 3)", desc: "Consignación de mercadería — sin IVA" },
]
...
export default function EditarGuiaPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/guias/${id}`).then((r) => r.json()) as Promise<ApiResponse<{
        ...
        patente: string | null
        items: { descripcion: string; cantidad: number; precio_unitario: number; descuento: number; producto_id: string | null }[]
      }>>,
      ...
      if (g.data) {
        setTipoTraslado((g.data.tipo_traslado as 1 | 2 | 3) ?? 1)
        setClienteId(g.data.cliente_id ?? "")
        setFecha(String(g.data.fecha).split("T")[0])
        setDireccionDestino(g.data.direccion_destino ?? "")
        setTransportistaRut(g.data.transportista_rut ?? "")
        setTransportistaNombre(g.data.transportista_nombre ?? "")
        setPatente(g.data.patente ?? "")
        if (g.data.items && g.data.items.length > 0) {
          setItems(g.data.items.map((i) => ({
            key: genKey(),
  ...
  async function handleEmitir() {
    ...
    setEmitting(true)
    try {
      const res1 = await fetch(`/api/guias/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(buildPayload()),
      })
      ...
      const res2 = await fetch(`/api/guias/${id}/emitir`, { method: "POST" })
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div>
        <h1 className="text-page-title text-sl-text">Editar Guía de Despacho</h1>
        <p className="mt-0.5 text-sm text-sl-muted">Modificar borrador de guía tipo 52</p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-sl-muted">
              Receptor{" "}
              {tipoTraslado !== 1 && (
                <span className="font-normal text-sl-muted/60">(opcional)</span>
              )}
            </label>
            <select
              value={clienteId}
      ...
      <div className="space-y-4 rounded-card border border-sl-border bg-sl-bg-card p-5">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-sl-text">
            Datos de transporte <span className="text-xs font-normal text-sl-muted">(opcionales)</span>
          </h2>
        </div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-sl-border px-4 py-2.5 text-sm text-sl-muted transition-colors hover:border-sl-purple/50 hover:text-sl-purple-light disabled:opacity-50"
          >
