...
export default function ProveedoresPage() {
  ...
  const load = useCallback(async (q?: string) => {
    setLoading(true)
    try {
      const res = await fetch(q ? `/api/proveedores?search=${encodeURIComponent(q)}` : "/api/proveedores")
      const json = await res.json()
      if (res.ok) setProveedores(json.data ?? [])
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  async function save() {
    ...
    try {
      ...
      const url = modal === "edit" ? `/api/proveedores/${editingId}` : "/api/proveedores"
      ...
      notify(modal === "edit" ? "Proveedor actualizado" : "Proveedor creado", true)
      closeModal()
      load(search)
    } finally {
      setSaving(false)
  ...
  async function del(id: string) {
    ...
    try {
      ...
      notify("Proveedor eliminado", true)
      load(search)
    } finally {
      setDeletingId(null)
    }
  ...
  async function importCsv(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    notify(
      `${importados} proveedor${importados !== 1 ? "es" : ""} importado${importados !== 1 ? "s" : ""}${errores.length > 0 ? ` (${errores.length} error${errores.length !== 1 ? "es" : ""})` : ""}`,
      errores.length === 0
    )
    load(search)
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-page-title text-sl-text">Proveedores</h1>
          <p className="mt-0.5 text-sm text-sl-muted">Gestión del catálogo de proveedores</p>
        </div>
      ...
      <div className="overflow-hidden rounded-card border border-sl-border bg-sl-bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="py-12 text-center text-sm text-sl-muted">Cargando...</td>
              </tr>
            ) : proveedores.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-12 text-center text-sm text-sl-muted">
                  {search ? "No se encontraron proveedores con esa búsqueda" : "No hay proveedores registrados aún"}
                </td>
              </tr>
            ) : (
              proveedores.map((p) => (
                ...
                >
                  <td className="px-4 py-3 font-mono text-xs text-sl-muted">{formatRutDisplay(p.rut)}</td>
                  <td className="px-4 py-3 font-medium text-sl-text">{p.razon_social}</td>
                  <td className="hidden px-4 py-3 text-sl-muted md:table-cell">{p.giro || "—"}</td>
                  <td className="hidden px-4 py-3 text-sl-muted lg:table-cell">{p.email || "—"}</td>
                  <td className="hidden px-4 py-3 text-sl-muted lg:table-cell">{p.telefono || "—"}</td>
            ...
            <div className="space-y-4 p-5">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-sl-muted">
                  RUT <span className="text-sl-danger">*</span>
                </label>
                <RutInput
                  value={form.rut}
              ...
              {field("Razón Social *", "razonSocial", { placeholder: "Distribuidora Ejemplo SpA" })}
              {field("Giro", "giro", { placeholder: "Distribución de productos" })}

              <div className="grid grid-cols-2 gap-3">
                {field("Email", "email", { type: "email", placeholder: "contacto@proveedor.cl" })}
                {field("Teléfono", "telefono", { placeholder: "+56 2 1234 5678" })}
              </div>

              {field("Dirección", "direccion", { placeholder: "Av. Industrial 456" })}
